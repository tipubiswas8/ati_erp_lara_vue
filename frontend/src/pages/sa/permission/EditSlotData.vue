<template>
  <AForm :model="formData" ref="formRef" @finish="onFinish">
    <ARow :style="{ flexDirection: 'column' }">
      <AFormItem name="permission_name" label="Permission Name"
        :rules="[{ required: true, message: 'Permission name is required' }]">
        <AInput v-model:value="formData.permission_name" autocomplete="off" />
        <a-label :style="{ color: '#780650' }">{{ fieldErrors.permission_name }}</a-label>
      </AFormItem>
...
const formData = reactive({
  permission_id: props.permissionData?.id || 0,
  permission_name: props.permissionData?.name || '',
  org_name: props.permissionData?.org_id || '',
  status: props.permissionData?.status === 1,
});
...
const orgsUrl = props.permissionData?.org_get_url;
...
const edit_url = props.permissionData?.edit_url + '/' + formData.permission_id;
  ...
  try {
    ...
    if (response.status === 204 || response.status === 200) {
      ...
      successResponseData.push({
        ...
        name: response.data.data.permission_name,
      });
