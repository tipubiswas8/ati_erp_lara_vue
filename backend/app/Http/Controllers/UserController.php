    ...
    public function status(Request $request)
    {
        ...
        if ($user) {
            try {
                ...
                $update = $user->save();
                if ($update) {
                    return response()->json(['status' => 200, 'message' => 'User Status Update Successfully!']);
                } else {
                    return response()->json(['status' => 300, 'message' => 'Unable to update status']);
                }
            } catch (Exception $e) {
                return response()->json(['status' => 500, 'message' => 'Unable to update user status, please try again!', 'error' => $e]);
            }
        }
    }
